...
*eg: About Us - main menu item. if Our Logo and Sponsors are set to Category: About Us 
...
<?php $menubar = $this->get_title_by_category('Public');	  
	  ...
	  <?php for($vari=0;$vari<count($menubar)&& $vari<=5;$vari++)
	  {?>  	<?php $submenu = $this->get_title_by_category(str_replace(' ', '_',$menubar[$vari]));
			  $submenuexists = false;
			  if(is_array($submenu)&& !(empty($submenu))):$submenuexists=true;endif;?>
			  <li><a <?php if($submenuexists): echo 'class="MenuBarItemSubmenu"'; endif;?>href="<?php echo $indexpath.htmlspecialchars( $menubar[$vari] ) ?>"><?php echo htmlspecialchars($menubar[$vari]) ?></a>
 ...
 <?php }?>
     ...
     <?php else: ?>
	  <?php echo $menubar; /* Allow raw HTML block to be defined by extensions */ 
 	  endif; ?>
			...
			<ul id="footer-icons" class="noprint">
				...
				<ul id="footer-<?php echo $category ?>">
					<?php foreach( $links as $link ): ?>
						...
						<li id="footer-<?php echo $category ?>-<?php echo $link ?>"><?php trim($this->html( $link )) ?></li>
						<?php endif; ?>
					<?php endforeach; ?>
				</ul>
				<?php endif; ?>
