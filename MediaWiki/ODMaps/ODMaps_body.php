...
class ODMaps {
	...
	public static function setup() {
		...
		$wgOut->addScript( '<script src="https://maps.googleapis.com/maps/api/js?sensor=false&libraries=places" type="text/javascript"></script>' );
	}
	...
	public static function expandOdMap() {
		...
		foreach( func_get_args() as $arg ) {
			if( !is_object( $arg ) && preg_match( "/^(\w+?)\s*=\s*(.*?)\s*$/s", $arg, $m ) ) {
				...
				if( $k == 'options' && substr( $v, 0, 1 ) == '{' ) $opt = array_merge( $opt, str_replace( "\n\n", '<br />', json_decode( $v, true ) ) );
				else $opt[$k] = $v;
			}
		}
